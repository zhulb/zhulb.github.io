(function($){
	...
	$.fn.ztree_fenlei_toc = function(options) {
		// 将defaults 和 options 参数合并到{}
		var opts = $.extend({},$.fn.ztree_fenlei_toc.defaults,options);
		return this.each(function() {
			opts._zTree = $(this);
			
			// 创建table of content，获取元数据_headers
			var base=getBaseUrl();//window.location.href.replace(/#md.*/,'#md/');
			...
			for(var i=1;i<opts._header_nodes.length;i++){
				if(opts._header_nodes[i]['isPackage']==1){
	...
	//定义默认
	$.fn.ztree_fenlei_toc.defaults = {
		_zTree: null,
		_headers: [],
		_header_offsets: [],
		_header_nodes: [{ id:-1, pId:-1, name:"分类",open:true}],
		debug: true,
		highlight_offset: 0,
